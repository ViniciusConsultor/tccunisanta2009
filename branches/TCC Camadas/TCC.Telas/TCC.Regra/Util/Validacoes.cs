...
namespace TCC.Regra.Util
{
    ...
    public static class Validacoes
    {
        ...
        public static void ValidaData(string DataSemFormatacao)
        {
            ...
            if (data[2] < ANOMINIMO)
            {
                throw new Exceptions.Validacoes.DataInvalidaException(TCC.Regra.Exceptions.Validacoes.TipoErroData.ano, data);
            }
            else if (data[1] > mes.Length || data[1] < 1)
            {
                throw new Exceptions.Validacoes.DataInvalidaException(TCC.Regra.Exceptions.Validacoes.TipoErroData.mes, data);
            }
            else if (data[0] > mes[data[1]] || data[0] < 1)
            {
                throw new Exceptions.Validacoes.DataInvalidaException(TCC.Regra.Exceptions.Validacoes.TipoErroData.dia, data);
            }
        }
