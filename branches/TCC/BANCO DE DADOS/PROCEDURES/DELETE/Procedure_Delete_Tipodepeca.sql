USE Megatechdatabase
IF OBJECT_ID('sp_delete_tipodepeca', 'P')IS NOT NULL
	DROP PROCEDURE sp_delete_tipodepeca;
GO

CREATE PROCEDURE sp_delete_tipodepeca
@id_tipo_peca INT
AS

BEGIN TRY
--verifica a exitencia do codigo recebido
IF EXISTS(select 1 from Tipodepeca where id_tipo_peca=@id_tipo_peca)
BEGIN
  --deleta logicamente dependencias existentes
  IF EXISTS(select 1 from peca where id_tipo_peca=@id_tipo_peca)
	BEGIN
		UPDATE peca SET flg_ativo = 0
		WHERE id_tipo_peca=@id_tipo_peca
	END	
--realiza a exclusao logicamente
UPDATE Tipodepeca SET flg_ativo = 0
WHERE id_tipo_peca=@id_tipo_peca

END
ELSE
--retorna erro se não houver o codigo
RAISERROR('Tipo de peca inexistente!',16,1)
END TRY

BEGIN CATCH
DECLARE @ErrorMessage NVARCHAR(4000);
DECLARE @ErrorSeverity INT;
DECLARE @ErrorState INT;

SELECT
@ErrorMessage = ERROR_MESSAGE(),
@ErrorSeverity = ERROR_SEVERITY(),
@ErrorState = ERROR_STATE();
--Use RAISERROR inside the CATCH block to return error
--information about the original error that caused
--execution to jump to the CATCH block.
RAISERROR(
@ErrorMessage, --Message text
@ErrorSeverity, --Severity
@ErrorState --State
);
END CATCH;